...
async def test_upload_empty_file_returns_400(client):
    """Uploading an empty (but otherwise allowed) audio file returns 400."""
...
@pytest.mark.asyncio
async def test_upload_non_audio_rejected(client):
    """Non-audio uploads are rejected — this is an audio-only catalog."""
    from io import BytesIO
