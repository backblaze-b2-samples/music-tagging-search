...
| Index is exact cosine over a single `.npz` blob on B2 | Read-modify-write of the whole manifest per upsert; full scan per query — fine at catalog scale, slow at 100k+ tracks | Build a local LanceDB ANN table *from* the manifest for query (B2 transport unchanged), or shard the manifest | Low | Open |
| Index manifest "last write wins" under concurrent analyses | Two parallel analyses could clobber each other's upsert | Batch worker is sequential today; add a per-bucket lock or compare-and-swap if parallelizing | Low | Open |
