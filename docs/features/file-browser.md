...
## Purpose
List, preview, download, and delete **everything** in the Backblaze B2 bucket —
the full-bucket explorer (`tracks/`, `features/`, `index/`, and anything else). This
is distinct from the [Music Library](music-library.md), which is scoped to the
`tracks/` prefix and joins each track with its extracted features. The full-bucket
File Explorer is kept from the starter kit so you can inspect the derived feature
JSON and index artifacts directly.
